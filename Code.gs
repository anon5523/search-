...
var BACKEND_VER = 3;   // 版本號：診斷用，確認已部署新版
...
function doGet() { return json_({ ok: true, msg: '巡檢通報後端運作中', ver: BACKEND_VER }); }
...
function doPost(e) {
  ...
  try {
    ...
    if (body.type === 'ping')   return json_({ ok: true, msg: 'pong', ver: BACKEND_VER });
    ...
    if (body.type === 'pull')   return json_({ ok: true, ver: BACKEND_VER, projects: readProjects_(), pumps: readPumps_(), records: readRecords_() });
